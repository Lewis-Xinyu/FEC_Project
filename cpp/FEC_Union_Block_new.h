#pragma once
#ifndef PCL_SEGEMENT_FEC_UNION_BLOCK_H
#define PCL_SEGEMENT_FEC_UNION_BLOCK_H
...
struct FECUB_PointIndexTag {
    int point_index;
    int tag;
};

inline bool FECUB_TagLess(const FECUB_PointIndexTag& a, const FECUB_PointIndexTag& b) {
    return a.tag < b.tag;
}

struct FECUB_BlockKey {
    int x, y, z;
    bool operator==(const FECUB_BlockKey& other) const {
        return x == other.x && y == other.y && z == other.z;
    }
};

struct FECUB_BlockKeyHash {
    std::size_t operator()(const FECUB_BlockKey& k) const noexcept {
...
struct FECUB_BlockData {
    FECUB_BlockKey key{};
    std::vector<int> owner_indices;
    std::vector<int> expanded_indices;
};

class FECUB_DisjointSet {
public:
    FECUB_DisjointSet() = default;
    explicit FECUB_DisjointSet(int n) { reset(n); }
...
inline std::vector<FECUB_LocalCluster> FECUB_LocalClusterOnly(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
    const std::vector<int>& expanded_global_indices,
    const std::vector<int>& owner_global_indices,
    double tolorance,
    int max_n,
    FECUB_LocalStats& stats
) {
    using Clock = std::chrono::high_resolution_clock;

    std::vector<FECUB_LocalCluster> local_clusters;
    ...
    std::vector<FECUB_PointIndexTag> indices_tags(n);
    for (int i = 0; i < n; ++i) {
        indices_tags[i].point_index = i;
        indices_tags[i].tag = dsu.find(i);
    }
    std::sort(indices_tags.begin(), indices_tags.end(), FECUB_TagLess);

    int begin_index = 0;
    for (int i = 0; i < n; ++i) {
        if (indices_tags[i].tag != indices_tags[begin_index].tag) {
            FECUB_LocalCluster cluster;
    ...
    FECUB_LocalCluster last_cluster;
...
inline std::vector<pcl::PointIndices> FEC_Union_Block(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
    ...
    auto get_block_key = [&](const pcl::PointXYZ& pt) -> FECUB_BlockKey {
        return FECUB_BlockKey{
    ...
    std::unordered_map<FECUB_BlockKey, int, FECUB_BlockKeyHash> block_to_id;
    std::vector<FECUB_BlockData> blocks;
    blocks.reserve(cloud->size() / 256 + 1);

    for (int i = 0; i < static_cast<int>(cloud->size()); ++i) {
        FECUB_BlockKey key = get_block_key(cloud->points[i]);
        auto it = block_to_id.find(key);
        int bid;
        if (it == block_to_id.end()) {
            bid = static_cast<int>(blocks.size());
            block_to_id[key] = bid;
            blocks.push_back(FECUB_BlockData{key});
        } else {
            bid = it->second;
        }
    ...
    std::vector<std::vector<FECUB_LocalCluster>> block_local_clusters(blocks.size());
    std::vector<FECUB_LocalStats> block_stats(blocks.size());
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int bid = 0; bid < static_cast<int>(blocks.size()); ++bid) {
        FECUB_LocalStats stats;
        block_local_clusters[bid] = FECUB_LocalClusterOnly(
            cloud,
    ...
    std::vector<FECUB_LocalCluster> all_local_clusters;
    ...
    FECUB_DisjointSet dsu(static_cast<int>(all_local_clusters.size()));
    
    // 【核心修改点：彻底抛弃 unordered_map，改用定长 vector】
    // 利用点的全局索引(0 到 cloud->size()-1)天然连续的特性，实现 O(1) 内存访问，消除分配碎片
    std::vector<std::vector<int>> point_to_cluster_ids(cloud->size());

    // 写入逻辑不变，由于 point_to_cluster_ids 是预分配的，这里的 push_back 极快
    ...
    std::cout << std::fixed << std::setprecision(3)
              << "FEC_Union_Block_new total= " << total_ms << " ms "
              << "build= " << build_ms << " ms "
              << "search= " << search_ms << " ms "
              << "merge= " << merge_ms << " ms "
              << "final= " << final_ms << " ms "
