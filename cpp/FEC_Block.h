#pragma once
#ifndef PCL_SEGEMENT_FEC_BLOCK_H
#define PCL_SEGEMENT_FEC_BLOCK_H
...
struct FECB_PointIndexTag {
    int point_index;
    int tag;
};

inline bool FECB_TagLess(const FECB_PointIndexTag& a, const FECB_PointIndexTag& b) {
    return a.tag < b.tag;
}

struct FECB_BlockKey {
    int x, y, z;

    bool operator==(const FECB_BlockKey& other) const {
        return x == other.x && y == other.y && z == other.z;
    }
};

struct FECB_BlockKeyHash {
    std::size_t operator()(const FECB_BlockKey& k) const noexcept {
...
struct FECB_BlockData {
    FECB_BlockKey key{};
    std::vector<int> owner_indices;
    std::vector<int> expanded_indices;
};

class FECB_DisjointSet {
public:
    FECB_DisjointSet() = default;
    explicit FECB_DisjointSet(int n) { reset(n); }
...
inline std::vector<FECB_LocalCluster> FECB_LocalClusterOnly(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
    const std::vector<int>& expanded_global_indices,
    const std::vector<int>& owner_global_indices,
    double tolorance,
    int max_n,
    FECB_LocalStats& stats
) {
    using Clock = std::chrono::high_resolution_clock;

    std::vector<FECB_LocalCluster> local_clusters;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        auto tm0 = Clock::now();
        for (int idx : pointIdx) {
            int temp_tag_num = marked_indices[idx];
    ...
    std::vector<FECB_PointIndexTag> indices_tags(n);
    for (int i = 0; i < n; ++i) {
        indices_tags[i].point_index = i;
        indices_tags[i].tag = marked_indices[i];
    }
    std::sort(indices_tags.begin(), indices_tags.end(), FECB_TagLess);

    int begin_index = 0;
    for (int i = 0; i < n; ++i) {
        if (indices_tags[i].tag != indices_tags[begin_index].tag) {
            FECB_LocalCluster cluster;
    ...
    FECB_LocalCluster last_cluster;
...
inline std::vector<pcl::PointIndices> FEC_Block(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
    ...
    auto get_block_key = [&](const pcl::PointXYZ& pt) -> FECB_BlockKey {
        return FECB_BlockKey{
    ...
    std::unordered_map<FECB_BlockKey, int, FECB_BlockKeyHash> block_to_id;
    std::vector<FECB_BlockData> blocks;
    blocks.reserve(cloud->size() / 256 + 1);

    for (int i = 0; i < static_cast<int>(cloud->size()); ++i) {
        FECB_BlockKey key = get_block_key(cloud->points[i]);
        auto it = block_to_id.find(key);
        int bid;
        if (it == block_to_id.end()) {
            bid = static_cast<int>(blocks.size());
            block_to_id[key] = bid;
            blocks.push_back(FECB_BlockData{key});
        } else {
            bid = it->second;
        }
    ...
    std::vector<std::vector<FECB_LocalCluster>> block_local_clusters(blocks.size());
    std::vector<FECB_LocalStats> block_stats(blocks.size());
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int bid = 0; bid < static_cast<int>(blocks.size()); ++bid) {
        FECB_LocalStats stats;
        block_local_clusters[bid] = FECB_LocalClusterOnly(
            cloud,
    ...
    std::vector<FECB_LocalCluster> all_local_clusters;
    ...
    FECB_DisjointSet dsu(static_cast<int>(all_local_clusters.size()));
...
#endif // PCL_SEGEMENT_FEC_BLOCK_H
