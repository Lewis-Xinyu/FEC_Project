#pragma once
#ifndef PCL_SEGEMENT_FECUNION_BLOCK_H
#define PCL_SEGEMENT_FECUNION_BLOCK_H

#include <pcl/point_types.h>
#include <pcl/kdtree/kdtree_flann.h>
#include <pcl/PointIndices.h>
#include <pcl/point_cloud.h>

#include <vector>
#include <algorithm>
#include <unordered_map>
#include <unordered_set>
#include <chrono>
#include <iostream>
#include <iomanip>
#include <limits>
#include <cmath>
#include <numeric>

#ifdef _OPENMP
#include <omp.h>
#endif

// =========================
// 原始 FECunion 风格的标签并查集
// =========================
struct LabelUnionFind_FECunion_Block {
    std::vector<int> parent;
    std::vector<int> min_label;

    LabelUnionFind_FECunion_Block() = default;
    explicit LabelUnionFind_FECunion_Block(int n) { reset(n); }

    void reset(int n) {
        parent.resize(n);
        min_label.resize(n);
        for (int i = 0; i < n; ++i) {
            parent[i] = i;
            min_label[i] = i;
        }
    }

    int find(int x) {
        int root = x;
        while (parent[root] != root) {
            root = parent[root];
        }
        while (parent[x] != x) {
            int next = parent[x];
            parent[x] = root;
            x = next;
        }
        return root;
    }

    void unite(int a, int b) {
        int root_a = find(a);
        int root_b = find(b);
        if (root_a != root_b) {
            if (min_label[root_a] < min_label[root_b]) {
                parent[root_b] = root_a;
            } else {
                parent[root_a] = root_b;
                min_label[root_b] = std::min(min_label[root_a], min_label[root_b]);
            }
        }
    }

    int canonical_label(int x) {
        return min_label[find(x)];
    }
};

// =========================
// 块键
// =========================
struct FECunionBlockKey {
    int x, y, z;

    bool operator==(const FECunionBlockKey& other) const {
        return x == other.x && y == other.y && z == other.z;
    }
};

struct FECunionBlockKeyHash {
    std::size_t operator()(const FECunionBlockKey& k) const noexcept {
        std::size_t h1 = std::hash<int>{}(k.x);
        std::size_t h2 = std::hash<int>{}(k.y);
        std::size_t h3 = std::hash<int>{}(k.z);
        return h1 ^ (h2 << 1) ^ (h3 << 7);
    }
};

// =========================
// block 数据
// =========================
struct FECunionBlockData {
    FECunionBlockKey key{};
    std::vector<int> owner_indices;     // 真正属于该块的点
    std::vector<int> expanded_indices;  // 扩展后的局部点
};

// =========================
// 局部簇
// =========================
struct FECunionLocalCluster {
    std::vector<int> all_global_indices;   // 局部簇涉及的全局点
    std::vector<int> owner_global_indices; // 真正属于本块 owner 的点
};

// =========================
// 计时
// =========================
struct FECunionLocalStats {
    double build_ms = 0.0;
    double search_ms = 0.0;
    double merge_ms = 0.0;
    double final_ms = 0.0;
};

// =========================
// 用于块间簇合并的并查集
// =========================
class FECunionClusterDSU {
public:
    FECunionClusterDSU() = default;
    explicit FECunionClusterDSU(int n) { reset(n); }

    void reset(int n) {
        parent.resize(n);
        rank.assign(n, 0);
        std::iota(parent.begin(), parent.end(), 0);
    }

    int find(int x) {
        if (parent[x] != x) {
            parent[x] = find(parent[x]);
        }
        return parent[x];
    }

    void unite(int a, int b) {
        int ra = find(a);
        int rb = find(b);
        if (ra == rb) return;

        if (rank[ra] < rank[rb]) std::swap(ra, rb);
        parent[rb] = ra;
        if (rank[ra] == rank[rb]) ++rank[ra];
    }

private:
    std::vector<int> parent;
    std::vector<int> rank;
};

// =========================
// 每个 block 内部：尽量按原 FECunion 的逻辑做局部打标
// =========================
inline std::vector<FECunionLocalCluster> FECunion_LocalClusterOnly(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
    const std::vector<int>& expanded_global_indices,
    const std::vector<int>& owner_global_indices,
    double tolorance,
    int max_n,
    FECunionLocalStats& stats
) {
    using Clock = std::chrono::steady_clock;

    std::vector<FECunionLocalCluster> local_clusters;
    if (expanded_global_indices.empty()) {
        return local_clusters;
    }

    auto tb0 = Clock::now();

    pcl::PointCloud<pcl::PointXYZ>::Ptr local_cloud(new pcl::PointCloud<pcl::PointXYZ>);
    local_cloud->reserve(expanded_global_indices.size());
    for (int gidx : expanded_global_indices) {
        local_cloud->push_back(cloud->points[gidx]);
    }

    pcl::KdTreeFLANN<pcl::PointXYZ> kdtree;
    kdtree.setInputCloud(local_cloud);

    auto tb1 = Clock::now();
    stats.build_ms = std::chrono::duration<double, std::milli>(tb1 - tb0).count();

    const int n = static_cast<int>(expanded_global_indices.size());

    std::vector<int> marked_indices(n, 0);
    std::vector<int> pointIdx;
    std::vector<float> pointSquaredDistance;
    pointIdx.reserve(std::max(16, max_n));
    pointSquaredDistance.reserve(std::max(16, max_n));

    int tag_num = 1;
    LabelUnionFind_FECunion_Block tag_uf(n + 1);

    for (int i = 0; i < n; ++i) {
        if (marked_indices[i] != 0) continue;

        pointIdx.clear();
        pointSquaredDistance.clear();

        auto ts0 = Clock::now();
        kdtree.radiusSearch(local_cloud->points[i], tolorance, pointIdx, pointSquaredDistance, max_n);
        auto ts1 = Clock::now();
        stats.search_ms += std::chrono::duration<double, std::milli>(ts1 - ts0).count();

        int min_tag_num = tag_num;

        for (int local_idx : pointIdx) {
            if (marked_indices[local_idx] > 0) {
                int current_tag = tag_uf.canonical_label(marked_indices[local_idx]);
                if (current_tag < min_tag_num) {
                    min_tag_num = current_tag;
                }
            }
        }

        if (min_tag_num == tag_num) {
            ++tag_num;
        }

        auto tm0 = Clock::now();
        for (int local_idx : pointIdx) {
            if (marked_indices[local_idx] > 0) {
                tag_uf.unite(marked_indices[local_idx], min_tag_num);
            } else {
                marked_indices[local_idx] = min_tag_num;
            }
        }
        auto tm1 = Clock::now();
        stats.merge_ms += std::chrono::duration<double, std::milli>(tm1 - tm0).count();
    }

    auto tf0 = Clock::now();

    for (int i = 0; i < n; ++i) {
        if (marked_indices[i] > 0) {
            marked_indices[i] = tag_uf.canonical_label(marked_indices[i]);
        }
    }

    std::unordered_set<int> owner_set(owner_global_indices.begin(), owner_global_indices.end());

    std::vector<int> label_counts(tag_num, 0);
    for (int i = 0; i < n; ++i) {
        if (marked_indices[i] > 0) {
            ++label_counts[marked_indices[i]];
        }
    }

    std::vector<int> valid_labels;
    valid_labels.reserve(tag_num);
    for (int label = 1; label < tag_num; ++label) {
        if (label_counts[label] > 0) {
            valid_labels.push_back(label);
        }
    }

    std::sort(valid_labels.begin(), valid_labels.end());

    for (int label : valid_labels) {
        FECunionLocalCluster cluster;

        for (int local_idx = 0; local_idx < n; ++local_idx) {
            if (marked_indices[local_idx] == label) {
                int global_idx = expanded_global_indices[local_idx];
                cluster.all_global_indices.push_back(global_idx);
                if (owner_set.find(global_idx) != owner_set.end()) {
                    cluster.owner_global_indices.push_back(global_idx);
                }
            }
        }

        std::sort(cluster.all_global_indices.begin(), cluster.all_global_indices.end());
        cluster.all_global_indices.erase(
            std::unique(cluster.all_global_indices.begin(), cluster.all_global_indices.end()),
            cluster.all_global_indices.end()
        );

        std::sort(cluster.owner_global_indices.begin(), cluster.owner_global_indices.end());
        cluster.owner_global_indices.erase(
            std::unique(cluster.owner_global_indices.begin(), cluster.owner_global_indices.end()),
            cluster.owner_global_indices.end()
        );

        if (!cluster.all_global_indices.empty()) {
            local_clusters.push_back(std::move(cluster));
        }
    }

    auto tf1 = Clock::now();
    stats.final_ms = std::chrono::duration<double, std::milli>(tf1 - tf0).count();

    return local_clusters;
}

// =========================
// 分块多线程版本
// =========================
inline std::vector<pcl::PointIndices> FECunion_Block(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
    int min_component_size,
    double tolorance,
    int max_n
) {
    using Clock = std::chrono::steady_clock;

    auto t_total_begin = Clock::now();

    if (!cloud || cloud->empty() || cloud->size() < static_cast<std::size_t>(min_component_size)) {
        return {};
    }

    // -------------------------
    // 1) partition
    // -------------------------
    auto tp0 = Clock::now();

    float min_x = std::numeric_limits<float>::max();
    float min_y = std::numeric_limits<float>::max();
    float min_z = std::numeric_limits<float>::max();

    for (const auto& p : cloud->points) {
        min_x = std::min(min_x, p.x);
        min_y = std::min(min_y, p.y);
        min_z = std::min(min_z, p.z);
    }

    // 和你前面 block 版本保持同思路，块稍大于搜索半径，减少块数与边界开销
    const double block_size = std::max(tolorance * 8.0, tolorance + 1e-6);

    auto get_block_key = [&](const pcl::PointXYZ& pt) -> FECunionBlockKey {
        return FECunionBlockKey{
            static_cast<int>(std::floor((pt.x - min_x) / block_size)),
            static_cast<int>(std::floor((pt.y - min_y) / block_size)),
            static_cast<int>(std::floor((pt.z - min_z) / block_size))
        };
    };

    std::unordered_map<FECunionBlockKey, int, FECunionBlockKeyHash> block_to_id;
    std::vector<FECunionBlockData> blocks;
    blocks.reserve(cloud->size() / 256 + 1);

    for (int i = 0; i < static_cast<int>(cloud->size()); ++i) {
        FECunionBlockKey key = get_block_key(cloud->points[i]);

        auto it = block_to_id.find(key);
        int bid;
        if (it == block_to_id.end()) {
            bid = static_cast<int>(blocks.size());
            block_to_id[key] = bid;
            blocks.push_back(FECunionBlockData{key});
        } else {
            bid = it->second;
        }
        blocks[bid].owner_indices.push_back(i);
    }

    auto tp1 = Clock::now();
    const double partition_ms = std::chrono::duration<double, std::milli>(tp1 - tp0).count();

    // -------------------------
    // 2) expand
    // -------------------------
    auto te0 = Clock::now();

    for (int global_idx = 0; global_idx < static_cast<int>(cloud->size()); ++global_idx) {
        const auto& pt = cloud->points[global_idx];

        const int bx0 = static_cast<int>(std::floor((pt.x - min_x - tolorance) / block_size));
        const int by0 = static_cast<int>(std::floor((pt.y - min_y - tolorance) / block_size));
        const int bz0 = static_cast<int>(std::floor((pt.z - min_z - tolorance) / block_size));

        const int bx1 = static_cast<int>(std::floor((pt.x - min_x + tolorance) / block_size));
        const int by1 = static_cast<int>(std::floor((pt.y - min_y + tolorance) / block_size));
        const int bz1 = static_cast<int>(std::floor((pt.z - min_z + tolorance) / block_size));

        for (int bx = bx0; bx <= bx1; ++bx) {
            for (int by = by0; by <= by1; ++by) {
                for (int bz = bz0; bz <= bz1; ++bz) {
                    FECunionBlockKey key{bx, by, bz};
                    auto it = block_to_id.find(key);
                    if (it != block_to_id.end()) {
                        blocks[it->second].expanded_indices.push_back(global_idx);
                    }
                }
            }
        }
    }

    for (auto& block : blocks) {
        std::sort(block.expanded_indices.begin(), block.expanded_indices.end());
        block.expanded_indices.erase(
            std::unique(block.expanded_indices.begin(), block.expanded_indices.end()),
            block.expanded_indices.end()
        );
    }

    auto te1 = Clock::now();
    const double expand_ms = std::chrono::duration<double, std::milli>(te1 - te0).count();

    // -------------------------
    // 3) 每块局部聚类（并行）
    // -------------------------
    std::vector<std::vector<FECunionLocalCluster>> block_local_clusters(blocks.size());
    std::vector<FECunionLocalStats> block_stats(blocks.size());

#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic)
#endif
    for (int bid = 0; bid < static_cast<int>(blocks.size()); ++bid) {
        FECunionLocalStats stats;
        block_local_clusters[bid] = FECunion_LocalClusterOnly(
            cloud,
            blocks[bid].expanded_indices,
            blocks[bid].owner_indices,
            tolorance,
            max_n,
            stats
        );
        block_stats[bid] = stats;
    }

    double local_build_max = 0.0;
    double local_search_max = 0.0;
    double local_merge_max = 0.0;
    double local_final_max = 0.0;

    for (const auto& s : block_stats) {
        local_build_max = std::max(local_build_max, s.build_ms);
        local_search_max = std::max(local_search_max, s.search_ms);
        local_merge_max = std::max(local_merge_max, s.merge_ms);
        local_final_max = std::max(local_final_max, s.final_ms);
    }

    // -------------------------
    // 4) 收集所有局部簇
    // -------------------------
    std::vector<FECunionLocalCluster> all_local_clusters;
    all_local_clusters.reserve(blocks.size() * 2);

    for (auto& clusters : block_local_clusters) {
        for (auto& c : clusters) {
            all_local_clusters.push_back(std::move(c));
        }
    }

    // -------------------------
    // 5) 块间全局簇合并：共享点 -> 合并局部簇
    // -------------------------
    auto tg0 = Clock::now();

    FECunionClusterDSU dsu(static_cast<int>(all_local_clusters.size()));
    std::vector<std::vector<int>> point_to_cluster_ids(cloud->size());

    for (int cid = 0; cid < static_cast<int>(all_local_clusters.size()); ++cid) {
        for (int gidx : all_local_clusters[cid].all_global_indices) {
            point_to_cluster_ids[gidx].push_back(cid);
        }
    }

    for (auto& cluster_ids : point_to_cluster_ids) {
        if (cluster_ids.size() <= 1) continue;
        int first = cluster_ids.front();
        for (int i = 1; i < static_cast<int>(cluster_ids.size()); ++i) {
            dsu.unite(first, cluster_ids[i]);
        }
    }

    auto tg1 = Clock::now();
    const double global_merge_ms = std::chrono::duration<double, std::milli>(tg1 - tg0).count();

    // -------------------------
    // 6) 输出最终 cluster（只保留 owner 点，防重复）
    // -------------------------
    auto tf0 = Clock::now();

    std::vector<std::vector<int>> root_to_owner_points(all_local_clusters.size());

    for (int cid = 0; cid < static_cast<int>(all_local_clusters.size()); ++cid) {
        int root = dsu.find(cid);
        auto& dst = root_to_owner_points[root];
        const auto& src = all_local_clusters[cid].owner_global_indices;
        dst.insert(dst.end(), src.begin(), src.end());
    }

    std::vector<pcl::PointIndices> cluster_indices;
    cluster_indices.reserve(root_to_owner_points.size());

    for (auto& indices : root_to_owner_points) {
        if (indices.empty()) continue;

        std::sort(indices.begin(), indices.end());
        indices.erase(std::unique(indices.begin(), indices.end()), indices.end());

        if (static_cast<int>(indices.size()) >= min_component_size) {
            pcl::PointIndices inliers;
            inliers.indices = std::move(indices);
            cluster_indices.push_back(std::move(inliers));
        }
    }

    auto tf1 = Clock::now();
    const double output_final_ms = std::chrono::duration<double, std::milli>(tf1 - tf0).count();

    // -------------------------
    // 7) 汇总时间
    // -------------------------
    auto t_total_end = Clock::now();
    const double total_ms = std::chrono::duration<double, std::milli>(t_total_end - t_total_begin).count();

    // 与你前面 block 版本保持一致：build 含 partition/expand/局部build；
    // search 取局部最慢线程时间；merge = 局部merge最慢 + 全局merge；
    // final = 局部final最慢 + 输出整理。
    const double build_ms = partition_ms + expand_ms + local_build_max;
    const double search_ms = local_search_max;
    const double merge_ms = local_merge_max + global_merge_ms;
    const double final_ms = local_final_max + output_final_ms;

    std::cout << std::fixed << std::setprecision(3)
              << "FECunion_Block total= " << total_ms << " ms "
              << "build= " << build_ms << " ms "
              << "search= " << search_ms << " ms "
              << "merge= " << merge_ms << " ms "
              << "final= " << final_ms << " ms "
              << "clusters=" << cluster_indices.size() << "\n";

    return cluster_indices;
}

#endif
