...
struct FilteredFrame {
    ...
    std::vector<std::uint32_t> packed_labels;
    int raw_points = 0;
    int removed_ground_points = 0;
};
...
static void print_help() {
    cout << "SemanticPOSS no-ground clustering evaluation\n"
         << "Usage:\n"
         << "  ./cpp/build/semanticposs_noground_eval_run [options]\n\n"
         << "Options:\n"
         ...
         << "  --help                   Show this help\n\n"
         << "Ground handling:\n"
         << "  This executable removes SemanticPOSS ground points (semantic id 22)\n"
         << "  before clustering.\n\n"
         << "Examples:\n"
         << "  ./cpp/build/semanticposs_noground_eval_run --root /mnt/d/semanticposs --sequence 00 --frame 000046 --alg FEC_Union_Block_new2 --tol 0.2\n"
         << "  ./cpp/build/semanticposs_noground_eval_run --root /mnt/d/semanticposs --sequence 00 --all 1 --limit 20 --alg FEC_Union_Block_new2 --tol 0.2\n";
}
...
static std::filesystem::path make_report_path(
    ...
    const std::string file_name =
        "semanticposs_noground_" + sanitize_filename(args.sequence) + "_" +
...
static void write_markdown_report(
    ...
    long long total_raw_points = 0;
    long long total_removed_ground_points = 0;
    ...
    for (const auto& r : results) {
        total_raw_points += r.raw_points;
        total_removed_ground_points += r.removed_ground_points;
    ...
    out << "# SemanticPOSS No-Ground Experiment Report\n\n";
    out << "## Setup\n\n";
    out << "- Root: `" << args.root << "`\n";
    ...
    out << "- Total raw points: `" << total_raw_points << "`\n";
    out << "- Total removed ground points: `" << total_removed_ground_points << "`\n";
    out << "- Total remaining points: `" << total_points << "`\n";
    out << "- Ground semantic id removed before clustering: `" << kGroundSemanticId << "`\n";
    out << "- FPS definition: `FPS = 1000 / cluster_wall_ms`\n\n";

    out << "## Summary\n\n";
    out << "| Frames | Total Raw Points | Removed Ground Points | Remaining Points | Total GT | Total Pred Clusters | avg_mIoU | avg_mRecall | avg_mPrecision | gt_match_rate@0.3 | gt_match_rate@0.5 | avg_cluster_wall (ms) | avg_frame_FPS | min_frame_FPS | max_frame_FPS |\n";
    out << "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n";
    out << std::fixed << std::setprecision(3)
        << "|" << results.size()
        << "|" << total_raw_points
        << "|" << total_removed_ground_points
        << "|" << total_points
        << "|" << total_gt
        << "|" << total_pred
        << "|" << avg_miou
    ...
    out << "| Frame | Raw Points | Removed Ground | Remaining Points | GT | Pred Clusters | mIoU | mRecall | mPrecision | matched@0.3 | matched@0.5 | cluster_wall (ms) | FPS |\n";
    out << "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n";
    for (const auto& r : results) {
        out << std::fixed << std::setprecision(3)
            << "|" << r.frame_id
            << "|" << r.raw_points
            << "|" << r.removed_ground_points
            << "|" << r.total_points
            << "|" << r.gt_objects
...
static FilteredFrame load_semanticposs_noground_frame(
    const std::filesystem::path& bin_path,
    const std::filesystem::path& label_path
) {
    auto raw_cloud = load_velodyne_bin_cloud(bin_path);
    auto raw_labels = load_uint32_labels(label_path);
    if (raw_labels.size() != raw_cloud->size()) {
        throw std::runtime_error(
            "Point/label count mismatch: points=" + std::to_string(raw_cloud->size()) +
            " labels=" + std::to_string(raw_labels.size())
        );
    }

    FilteredFrame filtered;
    filtered.cloud = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());
    filtered.cloud->reserve(raw_cloud->size());
    filtered.packed_labels.reserve(raw_labels.size());
    filtered.raw_points = static_cast<int>(raw_cloud->size());
    ...
    for (int i = 0; i < static_cast<int>(raw_labels.size()); ++i) {
        ...
        if (semantic_id == kGroundSemanticId) {
            ++filtered.removed_ground_points;
            continue;
        }
        filtered.cloud->push_back(raw_cloud->points[i]);
        filtered.packed_labels.push_back(raw_labels[i]);
    }
...
static FrameEval evaluate_frame(const Args& args, const std::filesystem::path& sequence_root, const std::string& frame_id) {
    ...
    FilteredFrame filtered = load_semanticposs_noground_frame(bin_path, label_path);

    std::unordered_map<std::uint32_t, int> gt_key_to_index;
    std::vector<std::vector<int>> gt_point_lists;
    std::vector<int> point_to_gt(filtered.cloud->size(), -1);

    for (int i = 0; i < static_cast<int>(filtered.packed_labels.size()); ++i) {
        const std::uint32_t packed = filtered.packed_labels[i];
    ...
    auto clusters = run_algorithm(filtered.cloud, args.algorithm, args.min_cluster_size, args.tolerance, args.max_n);
    ...
    result.raw_points = filtered.raw_points;
    result.removed_ground_points = filtered.removed_ground_points;
    result.total_points = static_cast<int>(filtered.cloud->size());
    ...
    cout << std::fixed << std::setprecision(3)
         ...
         << " removed_ground=" << result.removed_ground_points
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        cout << "SemanticPOSS No-Ground Eval\n";
        ...
        cout << "Ground semantic id removed: " << kGroundSemanticId << "\n";
        ...
        cout << std::fixed << std::setprecision(3)
             ...
             << " total_removed_ground=" << total_removed_ground_points
