#pragma once
#ifndef PCL_SEGEMENT_FEC1_IMPROVED_BLOCK_H
#define PCL_SEGEMENT_FEC1_IMPROVED_BLOCK_H
...
struct FEC1B_PointIndexTag {
    int point_index;
    int tag;
};

inline bool FEC1B_TagLess(const FEC1B_PointIndexTag& a, const FEC1B_PointIndexTag& b) {
    return a.tag < b.tag;
}

struct FEC1B_BlockKey {
    int x, y, z;
    bool operator==(const FEC1B_BlockKey& other) const {
        return x == other.x && y == other.y && z == other.z;
    }
};

struct FEC1B_BlockKeyHash {
    std::size_t operator()(const FEC1B_BlockKey& k) const noexcept {
...
struct FEC1B_BlockData {
    FEC1B_BlockKey key{};
    std::vector<int> owner_indices;
    std::vector<int> expanded_indices;
};

class FEC1B_DisjointSet {
public:
    FEC1B_DisjointSet() = default;
    explicit FEC1B_DisjointSet(int n) { reset(n); }
...
inline std::vector<FEC1B_LocalCluster> FEC1B_LocalClusterOnly(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
    const std::vector<int>& expanded_global_indices,
    const std::vector<int>& owner_global_indices,
    double tolorance,
    int max_n,
    FEC1B_LocalStats& stats
) {
    using Clock = std::chrono::high_resolution_clock;

    std::vector<FEC1B_LocalCluster> local_clusters;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int idx : pointIdx) {
            int cur_tag = marked_indices[idx];
            if (cur_tag > 0 && cur_tag < min_tag_num) {
                min_tag_num = cur_tag;
            }
        }
    ...
    std::vector<FEC1B_PointIndexTag> indices_tags(n);
    for (int i = 0; i < n; ++i) {
        indices_tags[i].point_index = i;
        indices_tags[i].tag = marked_indices[i];
    }
    std::sort(indices_tags.begin(), indices_tags.end(), FEC1B_TagLess);

    int begin_index = 0;
    for (int i = 0; i < n; ++i) {
        if (indices_tags[i].tag != indices_tags[begin_index].tag) {
            FEC1B_LocalCluster cluster;
    ...
    FEC1B_LocalCluster last_cluster;
...
inline std::vector<pcl::PointIndices> FEC1_Block(
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
    ...
    auto get_block_key = [&](const pcl::PointXYZ& pt) -> FEC1B_BlockKey {
        return FEC1B_BlockKey{
    ...
    std::unordered_map<FEC1B_BlockKey, int, FEC1B_BlockKeyHash> block_to_id;
    std::vector<FEC1B_BlockData> blocks;
    blocks.reserve(cloud->size() / 256 + 1);

    for (int i = 0; i < static_cast<int>(cloud->size()); ++i) {
        FEC1B_BlockKey key = get_block_key(cloud->points[i]);
        auto it = block_to_id.find(key);
        int bid;
        if (it == block_to_id.end()) {
            bid = static_cast<int>(blocks.size());
            block_to_id[key] = bid;
            blocks.push_back(FEC1B_BlockData{key});
        } else {
            bid = it->second;
        }
    ...
    std::vector<std::vector<FEC1B_LocalCluster>> block_local_clusters(blocks.size());
    std::vector<FEC1B_LocalStats> block_stats(blocks.size());
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int bid = 0; bid < static_cast<int>(blocks.size()); ++bid) {
        FEC1B_LocalStats stats;
        block_local_clusters[bid] = FEC1B_LocalClusterOnly(
            cloud,
    ...
    std::vector<FEC1B_LocalCluster> all_local_clusters;
    ...
    FEC1B_DisjointSet dsu(static_cast<int>(all_local_clusters.size()));
...
#endif // PCL_SEGEMENT_FEC1_IMPROVED_BLOCK_H
