...
def main():
    ...
    # ==========================================
    print("🌲 正在构建 SciPy KD-Tree (用于对照实验)...")
    tree = cKDTree(points)

    print("⏳ 正在执行逐点扫描与并查集合并 (预计耗时 30~50 秒)...")
    
    for i in range(size):
        if i % 200000 == 0 and i > 0:
            print(f"   已扫描 {i} 个点...")
            
        # 故意放在 for 循环里，每次只查 1 个点（模拟原版的慢速调用）
        # k=max_n+1 是因为 cKDTree 也会把点自己(距离为0)查出来
        distances, idx = tree.query(points[i], k=max_n+1, distance_upper_bound=tolerance)
